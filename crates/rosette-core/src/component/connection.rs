...
/// # Ok::<(), rosette_core::PortValidationReason>(())
/// ```
...
mod tests {
    ...
    fn test_connect_transform_simple() {
        ...
        let wg2_in = Port::with_width("in", Point::new(0.0, 0.0), -Vector2::unit_x(), 0.5).unwrap();
    ...
    fn test_connect_transform_with_rotation() {
        ...
        let target_port =
            Port::with_width("out", Point::new(10.0, 0.0), Vector2::unit_y(), 0.5).unwrap();
        // component_port at origin facing -X (180 degrees) - standard waveguide input
        let component_port =
            Port::with_width("in", Point::new(0.0, 0.0), -Vector2::unit_x(), 0.5).unwrap();
