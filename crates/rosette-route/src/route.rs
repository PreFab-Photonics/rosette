...
use rosette_core::{
    Cell, CellValidationError, Layer, Point, Polygon, PolygonValidationReason, Port,
    PortValidationReason, Vector2,
};
...
/// Invalid input supplied to a route or its annotations.
#[derive(Debug, Clone, thiserror::Error, PartialEq, Eq)]
pub enum RouteValidationError {
    /// Bend radii must be finite wherever they are stored as annotations.
    #[error("bend radius is not finite")]
    NonFiniteBendRadius,
    /// Route bend radii must be positive.
    #[error("bend radius is not positive")]
    NonPositiveBendRadius,
    /// An automatically reduced bend's requested radius must be finite.
    #[error("requested bend radius is not finite")]
    NonFiniteRequestedBendRadius,
    /// Route and bend positions must be finite.
    #[error("position is not finite")]
    NonFinitePosition,
    /// Route endpoint angles must be finite.
    #[error("angle is not finite")]
    NonFiniteAngle,
    /// Route widths must be finite.
    #[error("width is not finite")]
    NonFiniteWidth,
    /// Route widths must be positive.
    #[error("width is not positive")]
    NonPositiveWidth,
    /// Stored route path lengths must be finite.
    #[error("path length is not finite")]
    NonFinitePathLength,
}
...
pub enum RouteBuildError {
    ...
    /// Generated route annotations violate route model invariants.
    #[error("generated route data is invalid: {0}")]
    Validation(#[from] RouteValidationError),
    /// Generated route geometry is not representable by the core model.
    #[error("failed to construct route polygon: {0}")]
    Polygon(#[from] PolygonValidationReason),
    /// A generated route port is not representable by the core model.
    #[error("failed to construct route port: {0}")]
    Port(#[from] PortValidationReason),
    /// The route could not be materialized as a core cell.
    #[error("failed to construct route cell: {0}")]
    Cell(#[from] CellValidationError),
}

/// Information about a single bend generated by a route.
#[derive(Debug, Clone, PartialEq)]
pub struct BendInfo {
    /// Effective bend radius after any automatic reduction.
    radius: f64,
    /// Location of the bend corner/waypoint in the route cell's coordinates.
    position: Point,
    /// Original requested radius, if the bend was automatically reduced.
    requested_radius: Option<f64>,
}

impl BendInfo {
    /// Create a bend annotation.
    pub fn new(radius: f64, position: Point) -> Result<Self, RouteValidationError> {
        validate_bend_radius(radius)?;
        if !position.is_finite() {
            return Err(RouteValidationError::NonFinitePosition);
        }
        Ok(Self {
            radius,
            position,
    ...
    /// Create an annotation for an automatically reduced bend.
    pub fn auto_reduced(
        radius: f64,
        position: Point,
        ...
        if !requested_radius.is_finite() {
            return Err(RouteValidationError::NonFiniteRequestedBendRadius);
        }
        if requested_radius <= 0.0 {
            return Err(RouteValidationError::NonPositiveBendRadius);
        }
        Ok(Self {
            radius,
            position,
    ...
    /// Bend corner/waypoint in the route cell's coordinates.
    pub fn position(&self) -> Point {
        self.position
    }

    /// Original requested radius, if the bend was automatically reduced.
    pub fn requested_radius(&self) -> Option<f64> {
        self.requested_radius
    }
}

/// Route diagnostics associated with a materialized cell.
#[derive(Debug, Clone, Default, PartialEq)]
pub struct RouteAnnotations {
    /// Total optical path length, when available.
    path_length: Option<f64>,
    /// Bend annotations in the cell's local coordinates.
    bends: Vec<BendInfo>,
    /// Warnings generated while routing.
    warnings: Vec<String>,
}

impl RouteAnnotations {
    /// Create validated route diagnostics for a materialized cell.
    pub fn new(
        path_length: Option<f64>,
        bends: Vec<BendInfo>,
        warnings: Vec<String>,
    ) -> Result<Self, RouteValidationError> {
        if path_length.is_some_and(|length| !length.is_finite()) {
            return Err(RouteValidationError::NonFinitePathLength);
        }
        Ok(Self {
            path_length,
            bends,
...
impl Route {
    ...
    pub fn with_width(mut self, width: f64) -> Result<Self, RouteValidationError> {
    ...
    pub fn with_bend_radius(mut self, radius: f64) -> Result<Self, RouteValidationError> {
        validate_bend_radius(radius)?;
    ...
    pub fn start_at(
        ...
        y: f64,
        angle: f64,
    ) -> Result<&mut Self, RouteValidationError> {
    ...
    /// * `x` - X coordinate
    /// * `y` - Y coordinate
    pub fn to(&mut self, x: f64, y: f64) -> Result<&mut Self, RouteValidationError> {
        validate_position(x, y)?;
    ...
    pub fn to_full(
        ...
        if let Some(bend_radius) = bend_radius {
            validate_bend_radius(bend_radius)?;
        }
    ...
    fn generate(&self) -> Result<RouteResult, RouteBuildError> {
        let points = self.build_path_points();
        let mut warnings = Vec::new();

        if points.len() < 2 {
            return Err(RouteBuildError::InsufficientPoints {
                points: points.len(),
            });
        }
        ...
        if !path_length.is_finite() {
            return Err(RouteValidationError::NonFinitePathLength.into());
        }
    ...
    ) -> Result<(Vec<Corner>, Vec<BendInfo>), RouteValidationError> {
    ...
    ) -> Result<Vec<Polygon>, PolygonValidationReason> {
    ...
    fn generate_segment_polygon(
        ...
        start_setback: f64,
        end_setback: f64,
    ) -> Result<Option<Polygon>, PolygonValidationReason> {
...
impl RouteResult {
    ...
    pub fn into_cell_with_annotations(
        self,
        name: &str,
    ) -> Result<(Cell, RouteAnnotations), RouteBuildError> {
        let Self {
            polygons,
            path_length,
            port_in,
        ...
        cell.add_port(port_in)?;
        cell.add_port(port_out)?;
        let annotations = RouteAnnotations::new(Some(path_length), bends, warnings)?;
        Ok((cell, annotations))
    }
}

fn validate_position(x: f64, y: f64) -> Result<(), RouteValidationError> {
...
fn validate_angle(angle: f64) -> Result<(), RouteValidationError> {
    if !angle.is_finite() {
        return Err(RouteValidationError::NonFiniteAngle);
    }
    Ok(())
}

fn validate_width(width: f64) -> Result<(), RouteValidationError> {
    if !width.is_finite() {
        return Err(RouteValidationError::NonFiniteWidth);
    }
    if width <= 0.0 {
        return Err(RouteValidationError::NonPositiveWidth);
    }
    Ok(())
}

fn validate_bend_radius(radius: f64) -> Result<(), RouteValidationError> {
    if !radius.is_finite() {
        return Err(RouteValidationError::NonFiniteBendRadius);
    }
    if radius <= 0.0 {
        return Err(RouteValidationError::NonPositiveBendRadius);
    }
...
mod tests {
    ...
    fn test_route_straight_line() {
        let layer = Layer::new(1, 0);
        let mut route = Route::new(layer).with_width(0.5).unwrap();

        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(100.0, 0.0).unwrap();
        route.end_at(100.0, 0.0, 0.0).unwrap();

        let result = route.generate().unwrap();
    ...
    fn width_transition() -> RouteResult {
        let mut route = Route::new(Layer::new(1, 0)).with_width(0.5).unwrap();
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to_full(20.0, 0.0, Some(1.5), None).unwrap();
        route.end_at(20.0, 0.0, 0.0).unwrap();
        route.generate().unwrap()
    }
    ...
    fn test_route_90_degree_bend() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(50.0, 0.0).unwrap();
        route.to(50.0, 50.0).unwrap();
        route.end_at(50.0, 50.0, PI / 2.0).unwrap();
    ...
    fn test_route_s_curve() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(25.0, 0.0).unwrap();
        route.to(25.0, 20.0).unwrap();
        route.to(50.0, 20.0).unwrap();
        route.end_at(50.0, 20.0, 0.0).unwrap();
    ...
    fn test_route_tight_corner_auto_reduce() {
        ...
        let mut route = Route::new(layer)
            .with_width(0.5)
            .unwrap()
            .with_bend_radius(20.0)
            .unwrap(); // Large radius

        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(10.0, 0.0).unwrap(); // Short segment
        route.to(10.0, 10.0).unwrap();
    ...
    fn test_route_port_to_port() {
        ...
        let port_a = Port::with_width("out", Point::new(0.0, 0.0), Vector2::unit_x(), 0.5).unwrap();
        let port_b =
            Port::with_width("in", Point::new(100.0, 50.0), -Vector2::unit_x(), 0.5).unwrap();

        let mut route = Route::new(layer).with_bend_radius(5.0).unwrap();
    ...
    fn test_route_to_cell() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(50.0, 0.0).unwrap();
        route.end_at(50.0, 0.0, 0.0).unwrap();
    ...
    #[test]
    fn test_route_constructors_return_typed_validation_errors() {
        assert!(matches!(
            Route::new(Layer::new(1, 0)).with_width(f64::INFINITY),
            Err(RouteValidationError::NonFiniteWidth)
        ));
        assert!(matches!(
            Route::new(Layer::new(1, 0)).with_width(0.0),
            Err(RouteValidationError::NonPositiveWidth)
        ));
        assert!(matches!(
            Route::new(Layer::new(1, 0)).with_bend_radius(f64::NAN),
            Err(RouteValidationError::NonFiniteBendRadius)
        ));
        assert!(matches!(
            Route::new(Layer::new(1, 0)).with_bend_radius(-1.0),
            Err(RouteValidationError::NonPositiveBendRadius)
        ));
        assert!(matches!(
            BendInfo::new(f64::NAN, Point::origin()),
            Err(RouteValidationError::NonFiniteBendRadius)
        ));
        assert!(matches!(
            BendInfo::new(0.0, Point::origin()),
            Err(RouteValidationError::NonPositiveBendRadius)
        ));
        assert!(matches!(
            BendInfo::new(5.0, Point::new(f64::INFINITY, 0.0)),
            Err(RouteValidationError::NonFinitePosition)
        ));
        assert!(matches!(
            BendInfo::auto_reduced(2.0, Point::origin(), f64::INFINITY),
            Err(RouteValidationError::NonFiniteRequestedBendRadius)
        ));
        assert!(matches!(
            RouteAnnotations::new(Some(f64::NAN), Vec::new(), Vec::new()),
            Err(RouteValidationError::NonFinitePathLength)
        ));
    }

    #[test]
    fn build_rejects_aggregate_path_length_overflow() {
        let coordinate = f64::MAX / 2.0;
        let mut route = Route::new(Layer::new(1, 0));
        route.start_at(-coordinate, 0.0, 0.0).unwrap();
        route.to(coordinate, 0.0).unwrap();
        route.end_at(-coordinate, 0.0, 0.0).unwrap();

        assert!(matches!(
            route.build(),
            Err(RouteBuildError::Validation(
                RouteValidationError::NonFinitePathLength
            ))
        ));
    }

    #[test]
    fn test_route_mutation_validation_is_atomic() {
        let mut route = Route::new(Layer::new(1, 0));
        route.start_at(1.0, 2.0, 0.5).unwrap();

        assert!(matches!(
            route.start_at(f64::NAN, 3.0, 1.0),
            Err(RouteValidationError::NonFinitePosition)
        ));
        assert_eq!(route.start_position, Some(Point::new(1.0, 2.0)));
        assert_eq!(route.start_angle, Some(0.5));
        assert!(matches!(
            route.start_at(3.0, 4.0, f64::INFINITY),
            Err(RouteValidationError::NonFiniteAngle)
        ));
        assert_eq!(route.start_position, Some(Point::new(1.0, 2.0)));
        assert_eq!(route.start_angle, Some(0.5));

        assert!(matches!(
            route.to(f64::INFINITY, 0.0),
            Err(RouteValidationError::NonFinitePosition)
        ));
        assert!(matches!(
            route.to_full(5.0, 6.0, Some(0.0), None),
            Err(RouteValidationError::NonPositiveWidth)
        ));
        assert!(matches!(
            route.to_full(5.0, 6.0, None, Some(f64::INFINITY)),
            Err(RouteValidationError::NonFiniteBendRadius)
        ));
        assert!(route.waypoints.is_empty());

        assert!(matches!(
            route.end_at(7.0, 8.0, f64::NAN),
            Err(RouteValidationError::NonFiniteAngle)
        ));
        assert_eq!(route.end_position, None);
        assert_eq!(route.end_angle, None);
    }

    #[test]
    fn test_generated_bend_validation_flows_through_build_error() {
        let mut route = Route::new(Layer::new(1, 0));
        route.start_at(0.0, 0.0, 0.0).unwrap();
        ...
        route.end_at(10.0, 10.0, 0.0).unwrap();

        assert!(matches!(
            route.build(),
            Err(RouteBuildError::Validation(
                RouteValidationError::NonFinitePosition
            ))
        ));
    }

    #[test]
    fn test_generated_annotations_validation_flows_through_build_error() {
        let mut route = Route::new(Layer::new(1, 0));
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.end_at(10.0, 0.0, 0.0).unwrap();
        let mut result = route.build().unwrap();
        result.path_length = f64::NAN;

        assert!(matches!(
            result.into_cell_with_annotations("route"),
            Err(RouteBuildError::Validation(
                RouteValidationError::NonFinitePathLength
            ))
        ));
    ...
    #[test]
    fn test_build_propagates_core_port_validation_error() {
        let mut route = Route::new(Layer::new(1, 0));
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.end_at(10.0, 0.0, 0.0).unwrap();
        route.start_angle = Some(f64::NAN);

        assert!(matches!(
            route.build(),
            Err(RouteBuildError::Port(
                PortValidationReason::NonFiniteDirection
            ))
        ));
    }

    #[test]
    fn test_build_propagates_core_polygon_validation_error() {
        let mut route = Route::new(Layer::new(1, 0));
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.end_at(10.0, 0.0, 0.0).unwrap();
        route.start_position = Some(Point::new(f64::NAN, 0.0));

        assert!(matches!(
            route.build(),
            Err(RouteBuildError::Polygon(
                PolygonValidationReason::NonFiniteVertex { .. }
            ))
        ));
    }

    #[test]
    fn test_materialization_propagates_core_cell_validation_error() {
        let mut route = Route::new(Layer::new(1, 0));
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.end_at(10.0, 0.0, 0.0).unwrap();

        assert!(matches!(
            route.build().unwrap().into_cell_with_annotations(""),
            Err(RouteBuildError::Cell(CellValidationError::EmptyCellName))
        ));
    }
    ...
    fn test_route_result_owns_layer_and_materialization() {
        ...
        let (cell, annotations) = result.into_cell_with_annotations("result").unwrap();
        assert_eq!(cell.polygons().next().unwrap().1, &layer);
        assert_eq!(annotations.path_length(), Some(10.0));
        assert!(annotations.bends().is_empty());
    ...
    fn test_route_euler_90_degree_bend_path_length() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(leg, 0.0).unwrap();
        route.to(leg, leg).unwrap();
        route.end_at(leg, leg, PI / 2.0).unwrap();
    ...
    fn test_route_euler_s_curve() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(40.0, 0.0).unwrap();
        route.to(40.0, 30.0).unwrap();
        route.to(80.0, 30.0).unwrap();
        route.end_at(80.0, 30.0, 0.0).unwrap();
    ...
    fn test_route_euler_polygon_is_simple_single_corner() {
        ...
        for turn_deg in [30.0_f64, 45.0, 60.0, 90.0, 120.0, 150.0] {
            for radius in [3.0_f64, 5.0, 10.0] {
                ...
                route.start_at(0.0, 0.0, 0.0).unwrap();
                route.to(run, 0.0).unwrap();
                route.end_at(exit_x, exit_y, theta).unwrap();
    ...
    fn test_route_euler_polygon_fits_corner_bounding_box() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(run, 0.0).unwrap();
        route.end_at(run, run, PI / 2.0).unwrap();
        let result = route.generate().unwrap();
        assert!(
            result.warnings.is_empty(),
            "unexpected warnings: {:?}",
    ...
    fn test_route_euler_total_arc_length_doubles_circular() {
        ...
        let make = |profile: BendProfile| {
            ...
            r.start_at(0.0, 0.0, 0.0).unwrap();
            r.to(50.0, 0.0).unwrap();
            r.to(50.0, 50.0).unwrap();
            r.end_at(50.0, 50.0, PI / 2.0).unwrap();
            r.generate().unwrap()
        };
    ...
    fn test_route_euler_polygon_count_and_density() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(100.0, 0.0).unwrap();
        route.to(100.0, 50.0).unwrap();
        route.end_at(100.0, 50.0, PI / 2.0).unwrap();
    ...
    fn test_route_euler_polygon_entry_exit_tangent() {
        ...
        route.start_at(0.0, 0.0, 0.0).unwrap();
        route.to(100.0, 0.0).unwrap();
        route.to(100.0, 100.0).unwrap();
        route.end_at(100.0, 100.0, PI / 2.0).unwrap();
