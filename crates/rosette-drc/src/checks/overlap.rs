...
pub fn check_forbid_overlap_bulk(
    ...
    let mut region2_cache: Vec<Option<Region>> = vec![None; polygons2.len()];
    ...
    for (poly1, orig_idx1) in polygons1 {
        ...
        for candidate in tree.locate_in_envelope_intersecting(&search_envelope) {
            ...
            let region1 = region1.get_or_insert_with(|| Region::from_polygon(poly1));
            let region2 =
                region2_cache[candidate.index].get_or_insert_with(|| Region::from_polygon(poly2));
...
fn check_forbid_overlap(
    ...
    let intersection = Region::from_polygon(poly1).intersect(&Region::from_polygon(poly2));
...
pub fn check_require_overlap_bulk(
    ...
    for (poly1, orig_idx1) in polygons1 {
        ...
        for candidate in tree.locate_in_envelope_intersecting(&search_envelope) {
            ...
            // (overlapping) case.
            if region1.intersect(region2).area() > 1e-10 {
                has_overlap = true;
                break;
            }
        }
...
fn check_require_overlap(
    ...
    // Check if intersection has any area
    let has_overlap = intersection.area() > 1e-10;
