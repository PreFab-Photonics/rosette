...
Six words. The agent reads `.rosette/api.pyi`, `rosette.toml`, and the `components/` directory, all before writing a single line of code. It finds `grating_coupler`, reads the docstring (port `"opt"` faces +X, grating body extends in -X, 127 um fiber pitch), and writes:
...
And then there's fabrication awareness: understanding how a design will actually be manufactured and how process variations affect performance. This is where [PreFab](https://prefabphotonics.com) comes in, and deeper DFM integration into the agent workflow is something we're actively working on.
