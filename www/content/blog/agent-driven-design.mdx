...
<Callout type="warn" title="Early days">
  Rosette is still early. The API is unstable, things will break, and there's a
  lot of ground to cover. If this direction resonates with you, the best way to
  follow along is to watch the [GitHub
  repo](https://github.com/prefab-photonics/rosette). We're building in the
  open. Contributions, feedback, and sharp questions are all welcome.
</Callout>
...
The same shift is happening in chip design. Specifically, photonic chip design, where the physical layout of waveguides, splitters, and couplers is increasingly defined in code rather than drawn by hand.

As chip development becomes more code-driven, agent-driven workflows follow naturally. That's what we're building for.

## Building for agents from the ground up
...
Any one of these on its own is useful. But the real leverage comes when all three come together: the agent has clear instructions, full context, _and_ a way to verify its own work.

Here's what this looks like in a Rosette project. When you run `rosette init` and pick your agent tool, the project receives a small context system:

**`AGENTS.md`** — the instruction file. It sends the agent to a compact task index, keeps design scripts compositional, and requires verification before the work is complete.

**`.rosette/index.md` and `.rosette/contracts/`** — the index routes layout, routing, verification, and component-authoring tasks to focused API contracts. Those contracts are derived from the complete `.rosette/api.pyi`, so signatures still have one source of truth without forcing every subsystem into the initial context.
...
**Focused skills** — actionable routing, verification, and component-authoring workflows installed where the selected agent harness can discover them.
...
It's worth noting what's _not_ on this list. No custom or fine-tuned LLM. No MCP servers. No dedicated agent harness. No paywall. Everything the agent needs is included in the project itself: plain files it can read with the tools it already has. This works with Claude Code, OpenCode, Cursor, or whatever comes next because we're not coupling to any particular agent runtime.

The philosophy is simple: give the agent the right instructions, context, and tools, and don't add unnecessary layers between it and the work.

This same pattern is showing up elsewhere. Vercel's engineering team [found](https://vercel.com/blog/how-to-build-agents-with-filesystems-and-bash) that replacing custom retrieval tooling with structured files and bash outperformed their previous approaches for agent context management. The domain is different, but the principle is the same: LLMs already know how to navigate filesystems. Give them well-structured files and they do the rest.
...
That's it. A single sentence. The agent reads `.rosette/index.md`, `rosette.toml`, and the focused layout and routing context, then produces a layout with multiple reusable child cells (waveguide sections, metal pads, hexagons, a ring, alignment crosses, triangles) placed across four layers with rotation, mirroring, routing, arc paths, and a custom star polygon. Here's a trimmed version of what it wrote:
...
design.add_ref(route.to_cell("s_bend_route").at(0, 0))

# Instance with transforms: rotation, mirroring
design.add_ref(pad_cell.at(0, 0).rotate(45).translate(180, -30))
design.add_ref(pad_cell.at(0, 0).mirror_x().translate(180, 30))
...
The agent builds it, runs checks, gets a few unconnected-port violations (expected for a geometry demo, not a circuit), fixes them by exposing those ports as top-level I/O, and runs again:
...
Twenty-two cell references, four layers, rotations, mirrors, routes, arcs, custom polygons, all from a one-sentence prompt. For purely geometric layouts, the workflow holds up well. The agent reads the API, writes the script, runs into issues, fixes them, and delivers a clean build.

Worth noting: this is the extreme end of vague prompts. The output exercises the API, but it's not something you'd fabricate. More specific intent produces more useful designs, as the next examples show.
...
Six words. The agent follows `.rosette/index.md` to the layout and routing contracts, then reads `rosette.toml` and the relevant `components/` source. It finds `grating_coupler`, reads the docstring (port `"opt"` faces +X, grating body extends in -X, 127 µm fiber pitch), and writes:
...
On its first attempt, the agent set `mid_x` too small. The build warned that the bend radius was auto-reduced from 10 µm to 4.5 µm. It read the warning, understood the geometry, increased the horizontal extent, and rebuilt clean. Then it ran checks:

```
$ uv run rosette check designs/loopback.py

  drc   23 rules, 57 polygons — passed
  checks   4 ports, 1 connections, 2 bends — passed
```

![Fiber loopback layout](/blog/agent-driven-design/loopback.png)

A few things to notice. The agent didn't hallucinate the API. It loaded the focused contracts generated from `.rosette/api.pyi`. It knew that grating couplers need 127 µm spacing because that detail lives in project-local component source. It caught a geometric issue in the bend radius, used the routing and verification workflows, and iterated without human intervention.

This is what "context" does. The agent reads the API, the constraints, and the conventions before writing a line of code.

## A 1x16 splitter tree

Now the slightly more complex case. A 1x16 MMI splitter tree: one input splits into 16 outputs via four stages of MMI 1x2 splitters, with grating couplers on all 17 ports, all aligned on the right side at 127 µm fiber pitch.
...
The agent reads the component library, understands the MMI port layout (`in` at the left, `out1`/`out2` at the right with 2 µm separation), and builds the tree stage by stage. Four stages of splitting, S-bend routes fanning out between each stage, and all 17 grating couplers placed in a column on the right at 127 µm pitch. The input GC at the bottom connects back to the first MMI with a U-turn route.

On the first pass, the output GCs were split between left and right sides. We asked the agent to make them all inline on the right, which meant updating the GC placement and route. It rewrote the layout, rebuilt, and checked:
...
```
uvx --from librosette rosette init my-design
cd my-design
```
