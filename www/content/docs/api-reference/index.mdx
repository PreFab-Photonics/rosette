...
<Card title={"DrcCache"} href={"/docs/api-reference/DrcCache"} />
...
<PyFunction name={"run_drc"} type={"(cell, rules, library=None, cache=None) -> DrcResult"}>
...
<PyParameter name={"cache"} type={"DrcCache | None"} value={"None"}>

Optional [`DrcCache`](./DrcCache) reused across calls to make re-runs
incremental (the `rosette serve` live-preview loop). Results are identical to
a cache-free run; only the amount of work differs.
...
Render only the named cell instead of the top cell.
