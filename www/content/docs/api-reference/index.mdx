...
<PyFunction name={"render_png"} type={"(library, *, bbox=None, cell=None, layers=None, width=1024, height=None, pad=0.1, bg='#1a1a1a', fill_alpha=178, palette=None) -> RenderResult"}>

Render a [`Library`](./Library) to a PNG image, returning a
[`RenderResult`](./RenderResult) with the image bytes and world↔pixel transform
metadata.

<div>

<PyParameter name={"library"} type={"Library"}>

The Library to render.
