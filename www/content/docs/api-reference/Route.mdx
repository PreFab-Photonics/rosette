...
inserting bends at corners and tapers for width transitions. The corner
shape is controlled by the `bend_profile` parameter: `"circular"`
...
<PyFunction name={"__init__"} type={"(layer, width=0.5, bend_radius=5.0, auto_taper=True, taper_length=10.0, bend_profile='circular') -> None"}>
...
<PyParameter name={"bend_profile"} type={"Literal['circular', 'euler']"} value={"'circular'"}>
...
<PyFunction name={"through"} type={"(*waypoints, layer, width=0.5, bend_radius=5.0, bend_profile='circular') -> Route"}>
