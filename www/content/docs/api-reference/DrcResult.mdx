...
from it through child-cell references). Violations with unknown cell-name
provenance are kept as a safe default. See [`Cell.drc_skip`](./Cell).
...
child-cell references. See [`Cell.drc_skip`](./Cell).

</PyAttribute>

<PyAttribute name={"waived_violations"} type={"int"}>

Number of violations suppressed by a region waiver
([`Cell.drc_waive_regions`](./Cell)).

A violation is waived when it was not already suppressed by `drc_skip` and
its location is fully contained within at least one waiver region
...
`drc_skip` suppression, not a waiver.
