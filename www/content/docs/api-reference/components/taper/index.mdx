...
<PyAttribute name={"__all__"} type={null} value={"['taper']"} />

<Tabs items={["Functions"]}>

<Tab value={"Functions"}>

<PyFunction name={"taper"} type={"(layer, width_in=0.5, width_out=1.0, length=10.0) -> Cell"}>
...
GDS layer for the taper geometry

</PyParameter>
<PyParameter name={"width_in"} type={"float"} value={"0.5"}>

Input width at x=0 in microns

</PyParameter>
<PyParameter name={"width_out"} type={"float"} value={"1.0"}>

Output width at x=length in microns

</PyParameter>
<PyParameter name={"length"} type={"float"} value={"10.0"}>

Length of the taper in microns
