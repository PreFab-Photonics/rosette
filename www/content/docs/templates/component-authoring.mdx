...
You can also place exported components visually. Run `uv run rosette serve` without
...
- Use `float`, `int`, `str`, `bool`, `Literal`, and optional `Layer` annotations for
  parameters that should be editable from the visual component form.
