...
└── .rosette/
    ├── api.pyi          # core API reference for agents
    └── cli.json         # complete machine-readable CLI contract
```
...
- **`AGENTS.md` / `CLAUDE.md`**: short instruction file the agent reads
  ...
  check after every change, and never hardcode layer numbers.
- **`.rosette/api.pyi`**: core API reference for the installed version
  of Rosette. It uses compact stub syntax for agent context, but is not
  an editor type stub. Component APIs live in the editable `components/`
  source instead.
- **`.rosette/cli.json`**: complete CLI contract for the installed version,
  including commands, flags, exit codes, and JSON output schemas.
...
`.rosette/api.pyi` and `.rosette/cli.json`. Do not rerun `rosette init` in an
existing project.
</Callout>
