...
<Callout title={"One placement model"} type={"info"}>
...
- Paths require at least two finite points and a finite nonzero width. Negative
  widths retain GDS absolute-width semantics.
...
selection. Unversioned core-struct JSON is not accepted. JSON and GDS import
reject malformed local model state, while export validates before writing. GDS
export also checks format-specific limits such as coordinate ranges and
representable reference transforms. Local validation does not require every
reference target to exist and does not reject hierarchy cycles; hierarchy
operations report those graph-level issues separately.
