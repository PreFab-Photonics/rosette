...
| --- | --- | --- |
| `--cell NAME` | selected/unique top or all roots | Render only this cell and its descendants. |
...
Import the same renderer with `from rosette.render import render_png`. Reach for
it when you already have a `Cell` or `Library` in memory and want to avoid the
subprocess hop.
...
See [Project configuration](/docs/guides/project-configuration#snapshots) for
the canonical `[snapshots]` schema.
