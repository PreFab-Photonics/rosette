...
```txt
my-chip/
├── rosette.toml          # Project config: layers, DRC rules, and checks
...
│   └── ...
├── .rosette/
│   ├── api.pyi           # Agent-readable core API reference
│   └── cli.json          # Machine-readable CLI contract
└── AGENTS.md             # AI agent instructions (if selected)
```

- **`rosette.toml`**: the central configuration file. It defines your project name,
  layer stack, and design-rule constraints. The **blank** template ships only a
  `[project]` section — add layers and rules as you need them.
...
- **`components/`**: a library of photonic building blocks (S-bends, MMIs, rings,
  ...
  freely.
- **`.rosette/api.pyi`**: the core Rosette API reference for agents. It uses
  stub syntax for compactness, but is not intended as an editor type stub.
- **`.rosette/cli.json`**: the complete machine-readable CLI contract, including
  commands, flags, exit codes, and JSON output schemas.

The `.rosette/` directory is gitignored because these references are tied to the
installed Rosette version. After a fresh clone, restore both files with
`uv run rosette update`. Do not rerun `rosette init` in an existing project.
