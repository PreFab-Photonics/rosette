...
Canonical imports::
    ...
    from rosette.drc import DrcPolicy, DrcResult, DrcRules, run_drc
...
class BendInfo:
    ...
    def radius(self) -> float:
        """Effective bend radius in microns."""
        ...
    @property
    def position(self) -> Point:
        """Route corner position."""
        ...
    @property
    def requested_radius(self) -> float | None:
        """Requested radius when auto-reduced, otherwise ``None``."""
        ...
...
class DrcPolicy:
    """Per-run DRC suppression policy keyed by cell name.

    Policies are separate from geometry and are passed explicitly to
    :func:`run_drc`. Waiver regions use the named cell's local coordinates.
    """

    def __init__(self) -> None: ...
    def skip_cell(self, /, cell_name: str) -> None:
        """Suppress violations attributed entirely to this cell or its subtree."""
        ...
    def waive_region(self, /, cell_name: str, region: BBox) -> None:
        """Waive violations fully contained by a cell-local region."""
        ...
    def skips(self, /, cell_name: str) -> bool:
        """Return whether the cell is explicitly skipped."""
        ...
    def waiver_regions(self, /, cell_name: str) -> list[BBox]:
        """Return a copy of the cell's local waiver regions."""
        ...
...
class DrcResult:
    ...
    """Number of violations suppressed by the run's :class:`DrcPolicy`.

    A violation is suppressed iff every cell it names is skipped (or is within
    the subtree of a skipped cell). Violations with unknown cell-name
    provenance are always kept.
    """
    skipped_cells: int
    ...
    A violation is waived iff it was not already suppressed by a cell skip
